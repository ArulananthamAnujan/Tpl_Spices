...
import { effectiveUnitCents, isWholesaleActive, isPromoActive } from '../lib/pricing';
            ...
            {items.map(item => (
              <div key={item.variation_id} className="bg-white rounded-2xl shadow-card p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {(() => {
                    const unit = effectiveUnitCents(item, item.quantity);
                    const discounted = unit < item.price_cents;
                    const wholesaleOn = isWholesaleActive(item, item.quantity);
                    return (
                      <div className="mt-1">
                        <span className={`font-bold text-sm ${discounted ? 'text-red-600' : 'text-tpl-forest'}`}>{formatPrice(unit)}</span>
                        {discounted && <span className="text-xs text-gray-400 line-through ml-1.5">{formatPrice(item.price_cents)}</span>}
                        {wholesaleOn && <span className="block text-[11px] font-semibold text-tpl-forest">Wholesale price ({item.wholesale_min_qty}+)</span>}
                        {!wholesaleOn && isPromoActive(item) && <span className="block text-[11px] font-semibold text-red-600">On sale</span>}
                        {!wholesaleOn && item.wholesale_price_cents != null && item.wholesale_min_qty != null && (
                          <span className="block text-[11px] text-tpl-forest/70">Add {Math.max(0, item.wholesale_min_qty - item.quantity)} more for wholesale</span>
                        )}
                      </div>
                    );
                  })()}
                ...
                <div className="text-right flex-shrink-0">
                  <p className="font-bold text-tpl-forest">{formatPrice(effectiveUnitCents(item, item.quantity) * item.quantity)}</p>
              ...
              <div className="space-y-2 mb-4 text-sm">
                {items.map(item => (
                  <div key={item.variation_id} className="flex justify-between text-gray-600">
                    ...
                    <span className="flex-shrink-0">{formatPrice(effectiveUnitCents(item, item.quantity) * item.quantity)}</span>
                  </div>
                ))}
              </div>
