...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: store, error: storeErr } = await supabaseAdmin
      .from("stores")
      .select("square_location_id, pickup_enabled, delivery_enabled")
      .eq("id", store_id)
      .maybeSingle();
    ...
    // Calculate totals
    const subtotalCents = items.reduce((sum, i) => sum + i.unit_price_cents * i.qty, 0);

    // Build Square line items
    const lineItems = items.map(item => ({
      quantity: String(item.qty),
      base_price_money: { amount: item.unit_price_cents, currency: "AUD" },
